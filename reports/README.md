...
The total coverage is 93%. We have a total of 136 statements. You can see the data in [this figure](figures/coverage.png)
...
![This figure](figures/Wand.png) The training and validation processes tracked several metrics, including loss, Intersection over Union (IoU), and Dice Coefficient, to evaluate the performance of the segmentation model. The loss curve reflects how well the model converges during training, while the IoU and Dice Coefficient offer detailed insights into the model's segmentation quality and accuracy. These metrics are particularly critical in image segmentation tasks, as they assess the model's capability to classify pixels accurately and produce precise segmentations. To ensure robust evaluation, we monitored these metrics alongside loss and accuracy throughout the training and validation phases.

Tracking these metrics allowed us to evaluate the model's progress, detect potential issues, and make adjustments to enhance performance. Moreover, to ensure transparency and reproducibility, we utilized Weights & Biases (W&B) to log hyperparameters and system metrics systematically. This logging process provided a detailed record of experiments, enabling us to document each iteration comprehensively. By combining effective metric tracking and experiment logging, we created a well-documented framework to optimize the model and ensure that all experimental outcomes were reproducible and informed by data-driven insights. This systematic approach was crucial to developing a reliable and accurate segmentation model.

Since these can be tracked from a container in the cloud, we were able to monitor the experiments in real time, and make decisions based on the results.
...
This is due to the fact that we run our model a few times on the Vertex AI, which is the most expensive service and since for that we need to use the artifact Registry the amount of that increases too. We also stored a lot of data in the cloud storage, which also contributed to the cost. The Cloud Run did not consume as much, as we did not have a lot of requests to the API.
...
The starting point of the diagram is our local machine, where we develop and integrate new features or updates. Whenever we commit code and push a pull request (PR) to the main branch in GitHub, it automatically triggers two workflows: the Test workflow and the Linting workflow. These workflows ensure the code quality and functionality are maintained. If the PR is approved and merged, it updates the GitHub repository's main branch.

When a push to the main branch occurs, it triggers the Deploy Train Model workflow, which interacts with the Google Cloud Artifact Registry to prepare artifacts for the training pipeline. These artifacts are then utilized by Vertex AI to train the model. The outputs of this training, including performance metrics and the trained model, are logged in Weights & Biases.

Weights & Biases triggers a webhook that initiates a Test Model Performance workflow in GitHub Actions. If the model meets performance standards, the workflow pushes the model to a Google Cloud Storage bucket.

In the deployment pipeline, the model is wrapped in a FastAPI application. GitHub Actions build and publish the FastAPI image to the Google Cloud Artifact Registry, which is then deployed to Cloud Run to serve predictions.
