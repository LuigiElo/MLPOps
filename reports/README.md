...
Different requirements files are used depending on its aplication and the environment, for example, `requirements.txt` for the main project, `requirements_dev.txt` for development, and `requirements_test.txt` for testing and `requirements_backend.txt` for the API.
...
--- From the cookiecutter template, we filled out the `src`, `data`, and `models` folders. The `src` folder contains the main codebase, including the model architecture, training script, and API implementation - but also a new folder called `config`containing config files for `hydra` and `vertex_ai` deployment. The `data` folder contains the dataset download and preprocessing scripts, while the `models` folder contains the model definition and training logic. We deviated from the template by adding a `utils` for some functions, and added a few extra files to `src/data` other than the `make_dataset`. This structure allowed us to organize the project components effectively and maintain a clear separation between the model development and deployment aspects. ---
